import 'package:flutter/material.dart';
import 'theme.dart';

enum Mood {
  good(
    label: 'Good',
    icon: Icons.sentiment_very_satisfied,
    color: MindfulnessTheme.softTeal,
  ),
  okay(
    label: 'Okay',
    icon: Icons.sentiment_neutral,
    color: MindfulnessTheme.skyBlue,
  ),
  bad(
    label: 'Bad',
    icon: Icons.sentiment_very_dissatisfied,
    color: MindfulnessTheme.mutedCoral,
  );

  const Mood({required this.label, required this.icon, required this.color});

  final String label;
  final IconData icon;
  final Color color;
}

class Quote {
  final String text;
  final String source;

  const Quote({required this.text, required this.source});
}

const List<Quote> quotes = [
  Quote(
    text:
        "God has not given me a spirit of fear, but of power and of love and of a sound mind.",
    source: "2 Timothy 1:7",
  ),
  Quote(
    text: "I can do all things through Christ who strengthens me.",
    source: "Philippians 4:13",
  ),
  Quote(text: "I am more than a conqueror.", source: "Romans 8:37"),
  Quote(
    text:
        "For I know the plans I have for you,” declares the LORD, “plans to prosper you and not to harm you, plans to give you hope and a future",
    source: "Jeremiah 29:11",
  ),
  Quote(
    text:
        "The two most important days in your life are the day you are born... and the day you find out why.",
    source: "Mark Twain",
  ),
  Quote(
    text:
        "Always be a first version of yourself and not a second rate version of someone else.",
    source: "Judy Garland",
  ),
  Quote(
    text:
        "Chains of habit are too light to be felt until they are too heavy to be broken.",
    source: "Warren Buffett",
  ),
  Quote(text: "Don’t dream of winning, train for it!", source: "Mo Farah"),
  Quote(
    text:
        "Sometimes, magic is just someone spending more time on something than anyone else might reasonably expect.",
    source: "Teller",
  ),
  Quote(
    text:
        "The direction you choose to face determines whether you’re standing at the end or the beginning of a road.",
    source: "Richelle E. Goodrich",
  ),
];

class VideoExcercise {
  final String title;
  final String youtubeId;

  const VideoExcercise({required this.title, required this.youtubeId});
}

const List<VideoExcercise> videoExercises = [
  VideoExcercise(title: "Be Present", youtubeId: "ZToicYcHIOU"),
  VideoExcercise(title: "Letting Go", youtubeId: "syx3a1_LeFo"),
  VideoExcercise(title: "Self Soothing", youtubeId: "Xl_B45DpMLU"),
  VideoExcercise(title: "Forgetting Time", youtubeId: "YF_P1ZzYgjA"),
  VideoExcercise(title: "Symphony", youtubeId: "fkgzQsxdn9I"),
  VideoExcercise(title: "Impermanence", youtubeId: "uumInvT4t9Y"),
  VideoExcercise(title: "Santosha", youtubeId: "5vI8Kh3-epg"),
  VideoExcercise(title: "Present", youtubeId: "lVx3mFxML80"),
  VideoExcercise(title: "Community", youtubeId: "F0CbKoWOors"),
  VideoExcercise(
    title: "A 5 Minute Mindfulness Meditation",
    youtubeId: "wGFog-OuFDM",
  ),
  VideoExcercise(title: "Body Scan Exercise", youtubeId: "15q-N-_kkrU"),
  VideoExcercise(title: "Just Breathe", youtubeId: "IyW4nwfwunw"),
];

class TextExcercise {
  final String title;
  final String text;

  const TextExcercise({required this.title, required this.text});
}

const List<TextExcercise> textExercises = [
  TextExcercise(
    title: "Body Scan Meditation",
    text:
        "Lie or sit comfortably, with your arms and legs relaxed.\n\n"
        "Direct your attention to your feet, noticing any sensations like pressure, temperature, or itches.\n\n"
        "Gradually move your awareness: to your lower legs, upper legs, abdomen, chest, and other parts of your body, observing any feelings without judgment.\n\n"
        "Allow your mind to wander: and then gently bring your attention back to the body part you're scanning.\n\n"
        "If you feel tension, acknowledge it and then let your attention move on to the next part of your body.",
  ),

  TextExcercise(
    title: "Guided Breathing",
    text:
        "Sit in a comfortable, upright position: with your eyes closed or your gaze soft.\n\n"
        "Take a few deep breaths, noticing how your body relaxes.\n\n"
        "Allow your breath to settle into a natural rhythm: and focus on the sensations of the breath as it flows in and out.\n\n"
        "When thoughts arise, gently acknowledge them: and bring your attention back to your breath, letting thoughts pass like waves. ",
  ),

  TextExcercise(
    title: "Mindful Walking",
    text:
        "Find a quiet place 10 to 20 feet in length to walk, such as a straight line or a peaceful outdoor area.\n\n"
        "Walk slowly and deliberately, focusing on the sensation of your feet touching the ground.\n\n"
        "Pay attention to your body's subtle movements that keep your balance and posture as you walk.\n\n"
        "When you reach the end of your path, turn and continue walking, maintaining awareness of your sensations.",
  ),

  TextExcercise(
    title: "Mindful Eating",
    text:
        "Before eating, take a moment to appreciate your food: by looking at it, smelling it, and observing its colors and textures.\n\n"
        "As you eat, focus on the experience: of tasting, chewing, and swallowing, paying attention to the flavors and sensations in your mouth.\n\n"
        "This can be done with any meal or even with a single piece of fruit.",
  ),

  TextExcercise(
    title: "Mindful Break",
    text:
        "Step away from your computer or phone to notice sounds, your heart rate, and the sensations in your body.",
  ),

  TextExcercise(
    title: "Five senses exercise",
    text:
        "Look around you and bring your attention to five things that you can see. Pick something that you don’t normally notice, like a shadow or a small crack in the concrete.\n\n"
        "Bring awareness to four things that you are currently feeling, like the texture of your pants, the feeling of the breeze on your skin, or the smooth surface of a table you are resting your hands on.\n\n"
        "Take a moment to listen, and note three things that you hear in the background. This can be the chirp of a bird, the hum of the refrigerator, or the faint sounds of traffic from a nearby road.\n\n"
        "Bring your awareness to two smells that you usually filter out, whether they’re pleasant or unpleasant. Perhaps the breeze is carrying a whiff of pine trees if you’re outside, or the smell of a fast-food restaurant across the street.\n\n"
        "Focus on one thing that you can taste right now, at this moment. You can take a sip of a drink, chew a piece of gum, eat something, notice the current taste in your mouth, or even open your mouth to search the air for a taste.",
  ),

  TextExcercise(
    title: "Breathing meditation",
    text:
        "Find a comfortable seated or lying position and close your eyes.\n\n"
        "Focus on the physical sensations of your breath as it enters and leaves your body, such as the feeling in your nostrils or the rise and fall of your abdomen.\n\n"
        "Don't force your breath; simply observe it.\n\n"
        "When your mind wanders, gently guide your attention back to your breath without judgment. ",
  ),

  TextExcercise(
    title: "Mindful Seeing",
    text:
        "Find a space at a window where there are sights to be seen outside\n\n"
        "Look at everything there is to see. Avoid labeling and categorizing what you see outside the window; instead of thinking “bird” or “stop sign,” try to notice the colors, the patterns, or the textures.\n\n"
        "Pay attention to the movement of the grass or leaves in the breeze. Notice the many different shapes present in this small segment of the world you can see. Try to see the world outside the window from the perspective of someone unfamiliar with these sights.\n\n"
        "Be observant, but not critical. Be aware, but not fixated.\n\n"
        "If you become distracted, gently pull your mind away from those thoughts and notice a color or shape again to put you back in the right frame of mind.",
  ),

  TextExcercise(
    title: "Sitting meditation",
    text:
        "Sit comfortably with your back straight, feet flat on the floor and hands in your lap.\n\n"
        "Breathing through your nose, focus on your breath moving in and out of your body.\n\n"
        "If physical sensations or thoughts interrupt your meditation, note the experience and then return your focus to your breath.",
  ),

  TextExcercise(
    title: "The Observer Meditation",
    text:
        "Take a comfortable seated position, and listen to the script\n\n"
        "Let yourself settle into your body and your mind.\n\n"
        "Try to let go of thoughts and clear the mind of its usual considerations.\n\n"
        "Focus your attention first to the room you’re sitting in. Picture yourself from the outside as you sit, exactly as an outsider might. Next, shift your attention inwards into your skin. Try and feel your skin as you are sitting in the chair.\n\n"
        "Try to envision the shape that your skin is making as you sit in contact with the chair, shifting your awareness toward any physical sensations you are experiencing. As you feel each one, acknowledge its existence before letting your consciousness let go of it and move on naturally.\n\n"
        "If you find any emotions coming up, recognize them and create space for them. Then bring your attention back to your observing self—your feelings and thoughts are there, but you are separate from them, noticing them. This is the “Observer you”.",
  ),

  TextExcercise(
    title: "The 3-Step Mindfulness Exercise",
    text:
        "Try to pause and take a comfortable but dignified posture. Notice the thoughts that come up and acknowledge your feelings, but let them pass. Attune yourself to who you are and your current state.\n\n"
        "The goal is to focus attention on one thing: your breath. Be aware of the movement of your body with each breath, of how your chest rises and falls, how your belly pushes in and out, and how your lungs expand and contract. Find the pattern of your breath and anchor yourself to the present with this awareness.\n\n"
        "Allow the awareness to expand out to your body. Notice the sensations you are experiencing, like tightness, aches, or perhaps a lightness in your face or shoulders. Keep in mind your body as a whole, as a complete vessel for your inner self.\n\n"
        "If you wish, you can then expand your awareness even further to the environment around you. Bring your attention to what is in front of you. Notice the colors, shapes, patterns, and textures of the objects you can see. Be present at this moment, in your awareness of your surroundings.\n\n"
        "When you are ready to finish the exercise, open your eyes slowly and try to carry that mindfulness with you as you go about your day.",
  ),
];

const soundFiles = [
  "ocean_waves.mp3",
  "birds_singing.mp3",
  "rainfall.mp3",
  "white_noise.mp3",
  "babbling_brook.mp3",
  "crickets.mp3",
  "waterfall.mp3",
  "leaves_rustling.mp3",
  "river_flowing.mp3",
  "brown_noise.mp3",
  "rain_thunder.mp3",
  "computer_hum.mp3",
];

const musicFiles = [
  "calm_meditation.mp3",
  "mindfulness_meditation.mp3",
  "soothing_drone.mp3",
  "healing_ringing.mp3",
  "spring_breeze.mp3",
  "muse_flight.mp3",
  "autumn_flight.mp3",
  "warm_day.mp3",
  "deep_relaxation.mp3",
  "mindfulness_relaxation.mp3",
  "forest_melody.mp3",
  "summer_rain.mp3",
  "deep_meditation.mp3",
  "path_to_harmony.mp3",
];
